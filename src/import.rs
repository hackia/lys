...
fn set_config(conn: &sqlite::Connection, key: &str, value: &str) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn import_from_git(
    ...
    for oid in commits_oids {
        ...
        let parent_hash = parent_oid
            .and_then(|p| git_map.get(&p).map(|(h, _)| h.clone()));
